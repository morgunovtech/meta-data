...
export async function onRequest({ request }: PagesEventContext): Promise<Response> {
  ...
  try {
    ...
    const street = addressComponents.road || addressComponents.pedestrian || addressComponents.cycleway;
    const houseNumber = addressComponents.house_number || addressComponents.house_name;

    const data = {
      address: result.display_name ?? 'Unknown address',
      country: addressComponents.country ?? 'Unknown',
      countryCode: addressComponents.country_code?.toUpperCase(),
