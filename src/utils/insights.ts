import type { HistoricalWeatherResult, ReverseGeocodeResult, TimezoneHolidayResult } from '../types/api';
...
export function resolveLocalTime(
  ...
  if (timezone) {
    return {
      iso: metadata?.shotDate ?? timezone.localTimeIso,
  ...
  if (metadata?.shotDate) {
    const timezoneFallback = deriveTimezoneFromOffset(metadata.shotOffsetMinutes);
    return { iso: metadata.shotDate, timezone: timezoneFallback };
  }
  return {};
}

function deriveTimezoneFromOffset(offsetMinutes?: number): string | undefined {
