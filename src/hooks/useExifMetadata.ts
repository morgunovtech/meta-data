  ...
  useEffect(() => {
    ...
    const parse = async () => {
      ...
      try {
        ...
        const orientationTag = parseOrientationTag(pickFirst<number | string>(exifRecord, ['Orientation', 'orientation']));
...
function parseOrientationTag(value: number | string | undefined): number | undefined {
  ...
  if (typeof value === 'string') {
    ...
    if (normalized.includes('90') && normalized.includes('cw')) {
      return 6;
    }
    if (normalized.includes('90') && normalized.includes('ccw')) {
      return 8;
    }
    if (normalized.includes('180')) {
      return 3;
    }
  }
  return undefined;
