...
const App: React.FC = () => {
  ...
  const faceDetections = useMemo(
    () => detections.filter((det) => det.category === 'face' && det.score >= 0.5),
    [detections]
  );
  const personDetections = useMemo(
    () => detections.filter((det) => det.label === 'person' && det.score >= 0.5),
    [detections]
  );
  ...
  const createProcessedCanvas = useCallback(async () => {
    ...
    if (blurFaces) {
      ...
      targets.forEach((det) => {
        const padX = Math.max(det.width * 0.2, 12);
        const padY = Math.max(det.height * 0.2, 12);
  ...
  }, [fileInfo, blurFaces, jpegQuality, blurStrength, createProcessedCanvas]);
  ...
  return (
    <div className="app-shell">
      ...
      <CleanupDownloadBlock
        ...
        estimatedSize={previewSizeBytes}
      />
      {notice ? (
